...
def UPDATESTATUS(MANDT):
    ...
    check_date = pd.read_sql_query(sql, con_cim)
    ...
    cim_log = pd.read_sql_query(sql, con_cim)
    ...
    #存入CIM暫存表
    cim_result.to_sql('sap_25a_temp', con=con_cim, if_exists='replace', index=False)
...
def READ_MES(MANDT):
    ...
    df_25A = pd.read_sql(sql, con_mes)

    sql = "EXECUTE MES_TEST.dbo.SAP_25B_ALL @BEGIN='"+BEGIN+"',@END='"+END+"'"
    df_25B = pd.read_sql(sql, con_mes)
...
for i,v in enumerate(AUFNR_LIST):
    ...
    checklog = pd.read_sql(sql, con_cim)
    ...
    if( (EXETYP_=='C' and STATUS_=='003') or (EXETYP_=='A' and (STATUS_=='002' or STATUS_=='004') ) or (EXETYP_=='' and STATUS_=='') ):
        ...
        print(sql)
        df_rmzhl = pd.read_sql(sql, con_cim)
...
#             因材料有重工造成資料重複~SAP不能丟重複值~故程式報錯~2023.5.9增加去重複步驟
            ...
            cim_25A["EXETYP"] = "A"
            cim_25A["STATUS"] = '001'
            cim_25A.to_sql('sap_25a', con=con_cim, if_exists='append', index=False)
