    ...
    try:
        if(del_lot_str!='' or len(del_lot)/1000>1):
            ...
            if(len(del_lot)/1000>1):            
                ...
                sql = "DELETE FROM MES.MES_PRODUCTLABELDATA WHERE "
                sql_del_lot=''
                for s in splitpart:    
...
#>0
            ...
            sql = sql_str('MES.MES_PRODUCTLABELDATA', cols)
...
#!=''
            ...
            sql = "DELETE FROM MES.MES_PRODUCTLABELDATA WHERE LOTNO in("+delete_lot_str+")"
            cur.execute(sql)
            eng_ris.commit()
        else:
